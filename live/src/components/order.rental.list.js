...
import {
  rentalQuotesGetListService,
  rentalQuotesSearchService,
...
export default function OrderList() {
  ...
  useEffect(() => {
    async function fetchData() {
      const orderList = await rentalQuotesGetListService(activePage);
      if (orderList.error) {
  ...
  const Order = (props) => (
    <tr>
      ...
      <td>
        ...
        {props.order["5.3"]}
        {","}
        {props.order["5.4"]} {props.order["5.5"]}
        {","}
        {props.order["5.6"]}
      </td>
      <td>
  ...
  const handleSearch = (e) => {
    ...
    if (
      ...
      async function fetchData() {
        ...
        const searchResult = await rentalQuotesSearchService(searchReq);
